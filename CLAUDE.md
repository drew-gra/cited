...
- Bench tools (`scripts/probe.ts`, `scripts/batch-summary.ts`) run the L1
  pipeline statelessly — no DB, no queue — for fast iteration on detectors.
...
| 4 — User-agent A/B probing | Pull 3–5 sample article URLs from the sitemap; fetch each with a baseline browser UA and with each AI bot UA; compare status / size / content hash. | 7d | **Pending** |
...
- **probe_log** — Layer 4 raw probe records. One row per (outlet, sample URL, UA, timestamp). Empty until L4 ships.
...
scripts/
  probe.ts                       — stateless L1/L2 bench probe (--brief, --l2)
...
**Done in v1:** L1, L2, L3, L5, platform detector, surgical refresh cache,
LayerVerdict shape, per-layer expandable evidence panels, methodology page,
IP rate limit, basePath rewrite-friendly URL handling.

**Pending in v1:**
- **L4 (UA probing)** — the largest remaining engineering lift. See the L4
  brief at the end of this file.
- **S4b (posture rule table)** — currently per-platform postures are
  derived from L1 only. The verdict shape from S4a is the input; S4b is
  the rule logic that combines verdicts into postures.
